...
// PushRef method
func (mOperandStack *OperandStack) PushRef(ref *heap.Object) {
	mOperandStack.slots[mOperandStack.size].ref = ref
	mOperandStack.size++
}

// PopRef method
func (mOperandStack *OperandStack) PopRef() *heap.Object {
	mOperandStack.size--
...
// PushSlot method
func (mOperandStack *OperandStack) PushSlot(slot Slot) {
	mOperandStack.slots[mOperandStack.size] = slot
	mOperandStack.size++
}

// PopSlot method
func (mOperandStack *OperandStack) PopSlot() Slot {
	mOperandStack.size--
	return mOperandStack.slots[mOperandStack.size]
}
func (mOperandStack *OperandStack) GetRefFromTop(n uint) *heap.Object {
	return mOperandStack.slots[mOperandStack.size-1-n].ref
}

func (mOperandStack *OperandStack) PushBoolean(val bool) {
	if val {
		mOperandStack.PushInt(1)
	} else {
		mOperandStack.PushInt(0)
	}
}
func (mOperandStack *OperandStack) PopBoolean() bool {
	return mOperandStack.PopInt() == 1
}

func (mOperandStack *OperandStack) Clear() {
	mOperandStack.size = 0
	for i := range mOperandStack.slots {
		mOperandStack.slots[i].ref = nil
	}
}
